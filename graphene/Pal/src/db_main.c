...
static void load_libraries(void) {
    ...
    if (len == 0)
        return;
    log_always("chenyp load library name");
    char* c = preload_str;
    char* library_name = c;
    for (;; c++) {
        if (*c == ',' || !*c) {
...
noreturn void pal_main(uint64_t instance_id,       /* current instance id */
    ...
    log_always("we are in pal main in src\n");
    if (!instance_id) {
