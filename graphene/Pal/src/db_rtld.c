...
static struct link_map* map_elf_object_by_handle(PAL_HANDLE handle, enum object_type type,
    ...
    log_always("map_elf_object_by_handle\n");
    struct link_map* l = new_elf_object(_DkStreamRealpath(handle), type);
    int ret;

    /* This is the ELF header.  We read it in `open_verify'.  */
    const ElfW(Ehdr)* header = (void*)fbp;  // Elf64_Ehdr
...
#define APPEND_WRITECOPY(prot) ((prot) | PAL_PROT_WRITECOPY)
    ...
    if (e_type == ET_DYN) {
        void* mapaddr = NULL;
        ret =
            _DkStreamMap(handle, (void**)&mapaddr, APPEND_WRITECOPY(c->prot), c->mapoff, maplength);
        if (ret < 0) {
        ...
        log_always("the begin and end address=%x\n", (int)l->l_map_start);
        log_always("the end of the address= %x\n", (int)l->l_map_end);
        if (has_holes)
...
#define ELFOSABI_LINUX 3 /* Linux. */
    ...
    if (memcmp(ehdr->e_ident, expected, EI_OSABI) != 0 ||
        (ehdr->e_ident[EI_OSABI] != ELFOSABI_SYSV && ehdr->e_ident[EI_OSABI] != ELFOSABI_LINUX)) {
        ret       = -PAL_ERROR_INVAL;
    ...
    size_t maplength = ehdr->e_phnum * sizeof(ElfW(Phdr));  // an array
    ...
    if (!g_loaded_maps) {
        ...
        while (end->l_next) end = end->l_next;
        end->l_next = map;
        map->l_prev = end;
    }
...
static inline __attribute_always_inline ElfW(Sym) *
    check_match(ElfW(Sym) * sym, ElfW(Sym) * ref, const char* undef_name, const char* strtab) {
...
static int relocate_elf_object(struct link_map* l) {
    if (l)
        log_always("relocate_elf_object = %s\n", l->l_name);
    struct textrels {
        ElfW(Addr) start;
        ElfW(Addr) len;
        int prot;
...
noreturn void start_execution(const char** arguments, const char** environs) {
    /* First we will try to run all the preloaded libraries which come with
       entry points */
    log_always("start_start_execution---------");
    int narguments = 0;
    ...
    for (int i = 0; arguments[i]; i++) cookies[cnt++] = (unsigned long int)arguments[i];
    cookies[cnt++] = 0;
    for (int i = 0; environs[i]; i++) cookies[cnt++] = (unsigned long int)environs[i];
    cookies[cnt++] = 0;
