...
static int load_enclave(struct pal_enclave* enclave, char* args, size_t args_size, char* env,
    ...
    log_always("chen_yp encalve->libpal_uri");
    log_always(enclave->libpal_uri);
    ...
    log_always("ecall_enclave_start\n");
    log_always(enclave->libpal_uri);
    log_always(args);
    // log_always(args_size);
    // log_always(env);
...
int main(int argc, char* argv[], char* envp[]) {
    ...
    log_debug("%d\n",first_process);
    log_debug("chenyp first_process");
    if (first_process) {
        log_debug("chen_yp Are we the first in this Graphene's namespace");
        ...
        log_always("Manifest file: %s", manifest_path);
