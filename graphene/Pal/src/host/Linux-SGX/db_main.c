...
noreturn void pal_linux_main(char* uptr_libpal_uri, size_t libpal_uri_len, char* uptr_args,
    ...
    log_always("chen we are in pal_linux_main in host");
    log_always(uptr_libpal_uri);
    log_always(uptr_args);
    ...
    log_always("chenyp_libpal_path=%s\n",libpal_path);
    /*
    ...
    log_always("init_slab_mgr");
    init_untrusted_slab_mgr();
    log_always("init_untrusted_slab_mgr");
    init_enclave_pages();
    log_always("init_enclave_pages");
    init_cpuid();

    log_always("setup_pal_map");
    /* now we can add a link map for PAL itself */
    setup_pal_map(&g_pal_map);

    /* initialize enclave properties */
    log_always("init_enclave");
    ret = init_enclave();
    if (ret) {
