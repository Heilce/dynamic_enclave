...
noreturn void* shim_init(int argc, void* args) {
    ...
    log_always("init_stack\n");
    RUN_INIT(init_elf_objects);
    log_always("init_elf_objects++++++++++++++ld.so\n");
    RUN_INIT(init_signal_handling);
    RUN_INIT(init_ipc_worker);
    log_always("init_ipc_worker++++++++++++++++++++++\n");
    if (g_pal_control->parent_process) {
    ...
    log_always("Shim process initialized");
     ...
     * `init_loader`. */
    log_always("execute_elf_object++++++++++after load bash");
    execute_elf_object(/*exec_map=*/NULL, new_argp, new_auxv);
    log_always("end**************execute_elf_object++++++++++after load bash");
    /* UNREACHABLE */
}
