...
static int load_elf_header(struct shim_handle* file, ElfW(Ehdr) * ehdr) {
    ...
    if (ret < 0) {
        errstring =
            "%s is not an ELF executable. Please note that Graphene doesn't support "
...
int load_elf_object(struct shim_handle* file, struct link_map** out_map) {
    ...
    if (!map) {
        log_error(
            "Failed to map %s. This may be caused by the binary being non-PIE, in which "
...
static int vdso_map_init(void) {
    ...
    int ret    = bkeep_mmap_any_aslr(ALLOC_ALIGN_UP(vdso_so_size), PROT_READ | PROT_EXEC,
                                     MAP_PRIVATE | MAP_ANONYMOUS, NULL, 0, LINUX_VDSO_FILENAME, &addr);
    if (ret < 0) {
        return ret;
    }
...
int register_library(const char* name, unsigned long load_address) {
    log_always("glibc register library %s loaded at 0x%08lx", name, load_address);
