...
long shim_do_read(int fd, void* buf, size_t count) {
    ...
    if (hdl->type == TYPE_SOCK) {
        put_handle(hdl);
        return shim_do_recvfrom(fd, buf, count, 0, NULL, NULL);  //chenyp modify
