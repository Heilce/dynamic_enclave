...
// Read callgraphs of hand-written assembly functions
// Maps a function name to a list of callees
std::string CallGraphs;
std::set<std::string> FuncPtrs;
...
namespace options {
enum OutputType { OT_NORMAL, OT_DISABLE, OT_BC_ONLY, OT_SAVE_TEMPS };
...
// Pwise option in gold plugin
static std::string fptr_option;
static std::string pwisecpp_option;
...
static void process_plugin_option(const char *opt_) {
  ...
  } else if (opt.startswith("pwise=")) {
    fptr_option = opt.substr(strlen("pwise="));
  } else if (opt.startswith("pwisecpp=")) {
    pwisecpp_option = opt.substr(strlen("pwisecpp="));
  } else {
...
// Read function pointers (in init and fini sections) from cg section
static unsigned getFuncPtrs(std::string CG) {
  // char *Str = (char *) const_cast<unsigned char *>(CG.bytes_begin());
  outs() << "get FuncPtrs\n";
  outs() << CG << "\n";
  char *Str = const_cast<char *>(CG.c_str());
  outs() << "the function pointer\n";
  outs() << Str;
  size_t size = *(size_t *)Str;
  Str += sizeof(size_t);
  while (size-- > 0) {
    std::string F(Str, strlen((char *)Str));
    FuncPtrs.insert(F);
    Str += strlen(Str) + 1;
  }
  return Str - CG.c_str();
...
static void processObject(ObjectFile *o) {
  outs() << "processObject";
  // outs()<<o->getFileName();
  if (o->isELF() && o->isRelocatableObject()) {
    auto *ELFBase = cast<ELFObjectFileBase>(cast<ObjectFile>(o));
    assert(ELFBase && "Failed to identify ELF file format");
    if (o->isLittleEndian()) {
      auto *ELF64LE = dyn_cast<ELF64LEObjectFile>(ELFBase);
      if (!ELF64LE)
        return;
      section_iterator I = ELF64LE->section_begin();
      section_iterator E = ELF64LE->section_end();
      for (; I != E; ++I) {
        StringRef SectionName;
        I->getName(SectionName);
        outs() << "get section name = ";
        outs() << SectionName << "\n  ";
        if (SectionName == ".cg") {
          outs() << "get the cg name\n";
          outs() << o->getFileName() << '\n';
          StringRef Content;
          I->getContents(Content);
          std::string ContentStr(Content.begin(), Content.end());
          unsigned offset = getFuncPtrs(ContentStr);
          CallGraphs.append(Content.begin() + offset, Content.end());
        }
      }
    } else {
      auto *ELF64BE = cast<ELF64BEObjectFile>(ELFBase);
    }
  }
}

// Process archives
static void processArchives(Archive *a) {
  Error Err = Error::success();
  for (auto &ErrorOrChild : a->children(Err)) {
    Expected<std::unique_ptr<Binary>> ChildOrErr = ErrorOrChild.getAsBinary();

    if (!ChildOrErr) {
      // message(LDPS_ERR, "Could not read from file : %s",
      //       toString(ChildOrErr.takeError()).c_str());
...
static ld_plugin_status claim_file_hook(const ld_plugin_input_file *file,
  ...
  *claimed = 1;
  ...
  if (!ObjOrErr) {
    ...
    StringRef Name(file->name);
    Expected<OwningBinary<Binary>> BinaryOrErr = createBinary(Name);
    if (!BinaryOrErr) {
      // message(LDPL_ERROR, "Could not read from file : %s",
      //     toString(BinaryOrErr.takeError()).c_str());
...
static std::unique_ptr<LTO> createLTO() {
  ...
  if (options::fptr_option == "full") {
    Conf.Options.PwiseMode = PwiseModeKind::FullScan;
  } else if (options::fptr_option == "pta") {
    Conf.Options.PwiseMode = PwiseModeKind::PtAnalysis;
  } else {
    Conf.Options.PwiseMode = PwiseModeKind::LocalScan;
  }
  if (options::pwisecpp_option == "all") {
    Conf.Options.PwiseCpp = PwiseCppKind::IncludeAll;
  } else if (options::pwisecpp_option == "pta") {
    Conf.Options.PwiseCpp = PwiseCppKind::CppPta;
  } else {
    Conf.Options.PwiseCpp = PwiseCppKind::ObjectSensitive;
  }
  Conf.CallGraphs = CallGraphs;
  Conf.FuncPtrs = FuncPtrs;
