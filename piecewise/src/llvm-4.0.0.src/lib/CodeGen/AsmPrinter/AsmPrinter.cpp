...
bool AsmPrinter::doInitialization(Module &M) {
  ...
  DGI = getAnalysisIfAvailable<DepGraphInfo>();
...
bool AsmPrinter::doFinalization(Module &M) {
  ...
  OutStreamer->SwitchSection(OutContext.getObjectFileInfo()->getDepSection());

  // Write CodePointers size
  std::ofstream code_point;
  code_point.open("CodePointers",std::ios::out|std::ios::app);
  std::map<Function *, int> *CodePointers = DGI->GetCodePointers();
  const MCExpr *CPSize =
      MCConstantExpr::create(CodePointers->size(), OutContext);
  OutStreamer->EmitValue(const_cast<MCExpr *>(CPSize), DL.getPointerSize());
  // Write pointers
  for (auto F : *CodePointers) {
    //code_point<<F.first->getName();
    OutStreamer->EmitSymbolValue(
        OutContext.getOrCreateSymbol(F.first->getName()), DL.getPointerSize());
  }
  std::ofstream call_graph;
  call_graph.open("Call_graph",std::ios::out|std::ios::app);
  // Write dependency graph
  OutStreamer->EmitIntValue(DGI->size(), DL.getPointerSize());  //0x04
  for (DepGraphInfo::iterator I = DGI->begin(); I != DGI->end(); I++) {
    MCSymbol *FnSymbol = getSymbol(I->first);
    
    OutStreamer->EmitBytes(I->first->getName());
    outs() << "AsmPrinter printf function Name() of caller"
           << I->first->getName() << '\n';
    call_graph<<"caller:";
    //call_graph<<(I->first->getName());
    ...
    // Emit fn start addr
    OutStreamer->EmitSymbolValue(
        OutContext.getOrCreateSymbol(I->first->getName()), DL.getPointerSize());
     outs()<<"offset\n";
     outs()<<OutContext.getOrCreateSymbol(I->first->getName())->getOffset()<<'\n';
    // Emit fn size
    if (FnSymbol != nullptr && FnSymbol->isDefined()) {
      OutStreamer->EmitValue(dyn_cast<MCSymbolELF>(FnSymbol)->getSize(),
                             DL.getPointerSize());
    } else {
      OutStreamer->EmitZeros(DL.getPointerSize());
    }

    // Emit size current function's dep list
    const MCExpr *ListSize =
        MCConstantExpr::create(I->second->size(), OutContext);
    OutStreamer->EmitValue(const_cast<MCExpr *>(ListSize), DL.getPointerSize());
    
    for (auto J = I->second->begin(); J != I->second->end(); J++) {
      Function *CurrentFn = J->first;
      // Emit name of func
      OutStreamer->EmitBytes(CurrentFn->getName());
      outs() << "the name of callee" << CurrentFn->getName() << "\n";
      OutStreamer->EmitZeros(1);
      // Emit start addr of current dep function
      OutStreamer->EmitSymbolValue(
          OutContext.getOrCreateSymbol(CurrentFn->getName()),
          DL.getPointerSize());
      auto FnSym = OutContext.lookupSymbol(CurrentFn->getName());
      // Emit size of function
      if (FnSym != nullptr && FnSym->isDefined()) {
        OutStreamer->EmitValue(dyn_cast<MCSymbolELF>(FnSym)->getSize(),
                               DL.getPointerSize());
      } else {
        OutStreamer->EmitZeros(DL.getPointerSize());
      }
  ...
  // Write Asm depgraphs
  outs() << "Write Asm depgraphs\n";
  OutStreamer->EmitIntValue(DGI->AsmCG.size(), DL.getPointerSize());
  for (DepGraphInfo::AsmCGTy::iterator I = DGI->AsmCG.begin();
       I != DGI->AsmCG.end(); I++) {
    outs() << "Asm depgraphs" << I->first.c_str();
    OutStreamer->EmitBytes(I->first.c_str());
    OutStreamer->EmitZeros(1);
    OutStreamer->EmitIntValue(I->second.size(), DL.getPointerSize());
    for (auto F : I->second) {
      OutStreamer->EmitBytes(F.c_str());
      OutStreamer->EmitZeros(1);
    }
  }

  // Write Functions will be invoked in init section
  OutStreamer->EmitIntValue(DGI->InitFuncs.size(), DL.getPointerSize());
  for (DepGraphInfo::InitFuncPtrTy::iterator I = DGI->InitFuncs.begin();
       I != DGI->InitFuncs.end(); I++) {
    OutStreamer->EmitBytes(I->c_str());
    OutStreamer->EmitZeros(1);
  }
...
GCMetadataPrinter *AsmPrinter::GetOrCreateGCPrinter(GCStrategy &S) {
  ...
  for (GCMetadataPrinterRegistry::iterator
           ...
           E = GCMetadataPrinterRegistry::end();
       I != E; ++I)
    if (Name == I->getName()) {
