...
private:
  ...
  std::map<MCSymbol *, std::vector<MCSymbol *>> Callgraphs;
  std::vector<MCSymbol *> FuncPtrs;
  MCSymbol *CurrentFunc;
...
public:
  ...
  MCSymbol *getCurrentFunc() override { return CurrentFunc; }
  void setCurrentFunc(MCSymbol *Func) override { CurrentFunc = Func; }
  void addCGEdge(MCSymbol *Caller, MCSymbol *Callee) override {
    Callgraphs[Caller].push_back(const_cast<MCSymbol *>(Callee));
  }
  void addCodePtr(MCSymbol *F) override { FuncPtrs.push_back(F); }
  void printCG() override {
    for (std::map<MCSymbol *, std::vector<MCSymbol *>>::iterator I =
             Callgraphs.begin();
         I != Callgraphs.end(); I++) {
      errs() << I->first->getName() << " calls ";
      for (auto F : I->second)
        errs() << F->getName() << " ";
      errs() << "\n";
    }
  ...
  void printCodePtr() override {
    for (std::vector<MCSymbol *>::iterator I = FuncPtrs.begin();
         I != FuncPtrs.end(); I++) {
      errs() << (*I)->getName() << " ";
    }
    errs() << "\n";
...
bool AsmParser::Run(bool NoInitialTextSection, bool NoFinalize) {
  ...
  MCSection *CurrentSection = Out.getCurrentSection().first;
  MCSection *CG = Ctx.getObjectFileInfo()->getCGSection();
  Out.SwitchSection(CG);
  outs() << "Write to CG section\n";
  // Write functions called by init
  Out.EmitIntValue(FuncPtrs.size(), Ctx.getAsmInfo()->getPointerSize());
  for (auto F : FuncPtrs) {
    outs() << "Write functions called by init\n";
    outs() << F->getName();
    Out.EmitBytes(F->getName());
    Out.EmitZeros(1);
  }
  // Write CG of asm funcs
  for (std::map<MCSymbol *, std::vector<MCSymbol *>>::iterator I =
           Callgraphs.begin();
       I != Callgraphs.end(); I++) {
    Out.EmitBytes(I->first->getName());
    Out.EmitZeros(1);
    const MCExpr *NCallee = MCConstantExpr::create(I->second.size(), Ctx);
    Out.EmitValue(const_cast<MCExpr *>(NCallee),
                  Ctx.getAsmInfo()->getPointerSize());
    for (auto F : I->second) {
      Out.EmitBytes(F->getName());
      Out.EmitZeros(1);
    }
  }
  Out.SwitchSection(CurrentSection);
