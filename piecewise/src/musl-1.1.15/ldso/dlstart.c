...
void _dlstart_c(size_t *sp, size_t *dynv)
{
	...
	//Skip env information on the stack
	for (i=argc+1; argv[i]; i++);
	size_t *auxv = (void *)(argv+i+1);

	//Read aux_vector stored on the stack
	...
	void * dep_addr = (char*) base + DEPOFF;
	mprotect(dep_addr, DEPSZ, PROT_READ | PROT_WRITE);

	//Perform relocation on libc.so using rel section
	...
	// Perform relocation on libc.so using rela section
	rel = (void *)(base+dyn[DT_RELA]);
	rel_size = dyn[DT_RELASZ];

	size_t *ro_addr = base + rel[0];
	...
	mprotect(dep_addr, DEPSZ, PROT_READ);
